...
async function runIssue(args) {
    ...
    if (!token)
        throw new Error("Set GITHUB_TOKEN, GH_TOKEN, or pass --token.");
...
async function runAction(args) {
    ...
    if (!eventPath)
        throw new Error("GITHUB_EVENT_PATH is required for action mode.");
    const event = JSON.parse(await readFile(eventPath, "utf8"));
    if (!event.issue?.title) {
        console.log("Issue Proof skipped: event does not contain an issue.");
        return;
    }
    const repository = flagString(args, "repo") ?? event.repository?.full_name ?? process.env.GITHUB_REPOSITORY;
    if (!repository)
        throw new Error("Repository is required for action mode.");
    const issue = {
    ...
    const minConfidence = Number(flagString(args, "min-confidence") ?? input("min-confidence") ?? "0.45");
    if (analysis.confidence < minConfidence) {
        console.log(`Issue Proof skipped: confidence ${analysis.confidence.toFixed(2)} is below ${minConfidence.toFixed(2)}.`);
        return;
    }
    if (args.flags.has("json")) {
    ...
    if (!issueNumber)
        throw new Error("Issue number is required for GitHub updates.");
    if (shouldPost) {
...
async function issueFromArgs(args) {
    const issuePath = flagString(args, "issue");
    if (issuePath) {
        const value = JSON.parse(await readFile(issuePath, "utf8"));
        if (!value.title)
            throw new Error(`${issuePath} must include a title.`);
        return value;
    }
...
function input(name) {
    ...
    return (process.env[`INPUT_${upperName.replace(/-/g, "_")}`] ??
        process.env[`INPUT_${upperName}`]);
}
function booleanInput(args, name, fallback) {

...
export function analyzeIssue(
  ...
  if (securityHits.length > 0 || hasSecretRedaction) {
    ...
    return {
      ...
      warnings,
      nextStep:
        "Move this discussion to a private security channel before asking for more public details.",
      redactedTitle,
      redactedBody,
      possibleDuplicates,
    };
  ...
  if (possibleDuplicates[0]?.similarity && possibleDuplicates[0].similarity >= 0.62) {
    ...
    evidence.push(
      `Similar issue found: ${formatDuplicate(possibleDuplicates[0])}.`,
    );
    return {
      ...
      warnings,
      nextStep:
        "Compare this report with the possible duplicate before requesting more information.",
      redactedTitle,
      redactedBody,
      possibleDuplicates,
    };
  ...
  if (hasReproduction) evidence.push("Reproduction steps were included.");
  else missingInfo.push("reproduction steps");

  if (hasExpected) evidence.push("Expected behavior was described.");
  else missingInfo.push("expected behavior");

  if (hasActual) evidence.push("Actual behavior or failure mode was described.");
  else missingInfo.push("actual behavior");

  if (hasEnvironment || hasVersion) evidence.push("Environment or version details were included.");
  else missingInfo.push("environment and version");
  ...
  if (looksLikeQuestion && !hasBugWords && missingInfo.length >= 3) {
    ...
    return {
      ...
      warnings,
      nextStep:
        "Route this to support or discussion unless the reporter can provide a concrete failing case.",
      redactedTitle,
      redactedBody,
      possibleDuplicates,
    };
  ...
  if (missingInfo.length >= 2 || fullText.trim().length < 80) {
    ...
    if (hasBugWords) suggestedLabels.add("bug");
    return {
      ...
      warnings,
      nextStep:
        "Ask the reporter for the missing details before assigning this to a maintainer.",
      redactedTitle,
      redactedBody,
      possibleDuplicates,
    };
...
function formatDuplicate(candidate: DuplicateCandidate): string {
